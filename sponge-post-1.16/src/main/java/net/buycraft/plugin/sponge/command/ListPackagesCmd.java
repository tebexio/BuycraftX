...
public class ListPackagesCmd implements CommandExecutor {
    ...
    public void sendPaginatedMessage(Node node, Audience source) {
        ...
        if(subcategories.size() > 0) {
            contents = subcategories.stream().map(category -> {
                return Component
                        ...
                        .clickEvent(ClickEvent.runCommand("/tebex packages " + category.getId()));
            ...
            for (Package p : node.getPackages()) {
                contents.add(
                    Component
                            ...
                            .append(Component.text(" - ").color(TextColor.color(Color.GRAY)))
                            ...
                            .clickEvent(ClickEvent.runCommand("/tebex checkout " + p.getId()))
                );
            }
        }
